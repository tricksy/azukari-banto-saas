window.SECTIONS = {
  "overview": `
      <section id="overview">
        ...
        <div class="stats">
          <div class="stat-card"><div class="number">2</div><div class="label">ユーザー種別</div></div>
        ...
        <table>
          ...
          <tr><td>ストレージ</td><td>Google Drive</td><td>Cloudflare R2</td></tr>
          <tr><td>認証</td><td>Worker PIN + Admin OAuth（2種別）</td><td>Worker PIN + Platform Admin OAuth（2種別）</td></tr>
          ...
          <tr><td>メール</td><td>GAS（Gmail）</td><td>Resend</td></tr>
        ...
        <table>
          ...
          <tr><td>マスタ管理</td><td>取引先・業者・担当者の管理</td><td>プラットフォーム管理者</td></tr>
          <tr><td>操作ログ</td><td>全操作の記録・追跡</td><td>プラットフォーム管理者</td></tr>
        ...
        <table>
          ...
          <tr><td>受付番号（親）</td><td>{担当者ID}-{YYYYMMDDHHmm}</td><td>TK7M-202601181430</td><td>同一受付の商品グルーピング</td></tr>
          <tr><td>預かり番号（子）</td><td>{担当者ID}-{YYYYMMDDHHmmss}-{連番2桁}</td><td>TK7M-20260118143025-01</td><td>商品1点ごとの管理</td></tr>
        </table>
      ...
      <section id="tech-stack">
        ...
        <table>
          ...
          <tr><td>google-auth-library</td><td>Google OAuth 2.0 IDトークン検証</td></tr>
        ...
        <table>
          ...
          <tr><td>Cloudflare R2</td><td>写真ストレージ</td></tr>
          <tr><td>Resend</td><td>メール送信（アラート通知等）</td></tr>
          <tr><td>Vercel Cron Jobs</td><td>定期アラートチェック・アーカイブ</td></tr>
        </table>
        ...
        <table>
          ...
          <tr><td>プラットフォーム管理者</td><td>Google OAuth 2.0</td><td>JWT Cookie（8時間）</td><td><code>platform_admins</code> テーブルで管理</td></tr>
        </table>
      </section>
`,
...
`,
  ...
  "database": `
      <section id="database-overview">
        ...
        <h3>スキーマ概要</h3>
        <div class="stats">
          <div class="stat-card"><div class="number">13</div><div class="label">テーブル数</div></div>
          <div class="stat-card"><div class="number">9</div><div class="label">ENUM定義</div></div>
        ...
        <table>
          ...
          <tr><td><code>log_target_type</code></td><td>item, reception, customer, partner, vendor, worker, auth, system, email</td><td>操作対象種別</td></tr>
      ...
      <section id="database-tables">
        ...
        <table>
          ...
          <tr><td>redirect_url</td><td>TEXT</td><td></td><td>テナント分離先URL（Premiumプラン用）</td></tr>
        ...
        <table>
          ...
          <tr><td>worker_id</td><td>VARCHAR(20)</td><td>UNIQUE(tenant_id, worker_id)</td><td>担当者ID（TK7M等）</td></tr>
        ...
        <h3>operation_logs（操作ログ）</h3>
        <table>
          <tr><th>カラム</th><th>型</th><th>説明</th></tr>
          <tr><td>id</td><td>UUID</td><td>内部ID（PK）</td></tr>
          <tr><td>tenant_id</td><td>UUID</td><td>FK -> tenants(id)</td></tr>
          <tr><td>worker_id</td><td>TEXT</td><td>担当者ID</td></tr>
          <tr><td>worker_name</td><td>TEXT</td><td>担当者名</td></tr>
          <tr><td>action</td><td>log_action</td><td>アクション種別（ENUM型）</td></tr>
          <tr><td>target_type</td><td>log_target_type</td><td>対象種別（ENUM型）</td></tr>
          <tr><td>target_id</td><td>TEXT</td><td>対象ID</td></tr>
          <tr><td>changes</td><td>JSONB</td><td>変更内容（JSON形式）</td></tr>
          <tr><td>is_archived</td><td>BOOLEAN</td><td>アーカイブ済み</td></tr>
          <tr><td>created_at</td><td>TIMESTAMPTZ</td><td>作成日時</td></tr>
        </table>

        <h3>email_logs（メール送信ログ）</h3>
        <table>
          <tr><th>カラム</th><th>型</th><th>説明</th></tr>
          <tr><td>id</td><td>UUID</td><td>内部ID（PK）</td></tr>
          <tr><td>tenant_id</td><td>UUID</td><td>FK -> tenants(id)</td></tr>
          <tr><td>to_email</td><td>TEXT</td><td>送信先メールアドレス</td></tr>
          <tr><td>subject</td><td>TEXT</td><td>件名</td></tr>
          <tr><td>body</td><td>TEXT</td><td>本文</td></tr>
          <tr><td>status</td><td>TEXT</td><td>送信ステータス（sent / failed）</td></tr>
          <tr><td>error</td><td>TEXT</td><td>エラーメッセージ（失敗時）</td></tr>
          <tr><td>sent_at</td><td>TIMESTAMPTZ</td><td>送信日時</td></tr>
          <tr><td>created_at</td><td>TIMESTAMPTZ</td><td>作成日時</td></tr>
        </table>
        ...
        <table>
          ...
          <tr><td><code>partners</code></td><td>partner_code, partner_name, phone, email, address</td><td>取引先マスタ</td></tr>
          ...
          <tr><td><code>claims</code></td><td>item_number, status, category, description</td><td>クレーム管理</td></tr>
...
CREATE POLICY service_role_all ON items
  FOR ALL TO service_role USING (true);
        ...
        <div class="info">
          <strong>注意:</strong> <code>current_setting('app.tenant_id', true)</code> の第2引数 <code>true</code> は、セッション変数が未設定の場合にエラーではなくNULLを返すオプションです。これにより、セッション変数未設定時はどの行にもマッチせず安全にデータが保護されます。
        </div>
        ...
        <table>
          ...
          <tr><td>email_logs</td><td>適用</td><td>適用</td></tr>
        ...
        <table>
          ...
          <tr><td>Cron ジョブ</td><td>全テナントを横断してアラートチェック</td><td><code>api/cron/*</code></td></tr>
          <tr><td>プラットフォーム管理</td><td>テナント管理は横断アクセスが必要</td><td><code>api/admin/*</code></td></tr>
        </table>
  ...
  "development": `
      ...
      <section id="dev-environment">
        ...
        <table>
          ...
          <tr><td><code>NEXT_PUBLIC_BASE_DOMAIN</code></td><td>ベースドメイン（クライアント用）</td><td>kuratsugi.app（※現在未使用）</td></tr>
