...
- Google OAuth 2.0（Google Sign-Inボタン）
- 許可されたメールアドレスのみログイン可能
- 認証成功後、管理者セッション（JWT Cookie）を発行
...
| 2 | テナント管理 | /admin/tenants | 🏠 |
| 3 | ステータス一覧 | /admin/statuses | 📋 |
| 4 | クレーム管理 | /admin/claims | ⚠️ |
| 5 | 有料預かり | /admin/paid-storage | 📦 |
| 6 | 取引先管理 | /admin/partners | 🏢 |
| 7 | 業者管理 | /admin/vendors | 🏭 |
| 8 | 顧客管理 | /admin/customers | 👥 |
| 9 | 担当者管理 | /admin/workers | 👤 |
| 10 | 操作ログ | /admin/logs | 📝 |
| 11 | メール送信ログ | /admin/email-logs | ✉️ |
| 12 | 使い方マニュアル | /admin/manual | 📖 |
| 13 | システム設定 | /admin/settings | ⚙️ |
...
**ファイル:** `src/app/admin/(dashboard)/tenants/page.tsx`
**状態:** 実装済み

### 構成

- テナント一覧テーブル（テナントID、店舗名、プラン、ステータス、分離状態）
- 「+ 新規テナント」ボタン → 作成モーダル
- 各テナントの「編集」ボタン → 分離URL設定モーダル

### 機能

- テナントの一覧表示
- 新規テナントの作成（店舗名、テナントID: 4桁16進数、プラン選択）
- テナント分離URL（redirect_url）の設定・解除
