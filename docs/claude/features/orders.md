...
| 項目 | 必須 | 説明 |
| ---- | ---- | ---- |
| 発注先業者 | ○ | 発送先の業者を選択（業者情報はbg-shironeri背景で表示、住所フィールド分離） |
| 発送日 | ○ | 発送日を入力（今日以前） |
| 送り状番号 | × | 配送業者の送り状番号（任意） |
...
| エンドポイント | メソッド | 説明 | 状態 |
| -------------- | -------- | ---- | ---- |
| /api/items | GET | ステータス別商品一覧 | 実装済み |
| /api/items/[itemNumber] | PATCH | ステータス遷移・発送情報更新 | 実装済み |
...
- [x] pending_ship/rework の商品一覧取得
- [x] 業者選択ドロップダウン
- [x] 発送登録モーダル（発注先業者・日付・送り状番号）
- [x] 一括選択・操作UI
- [x] キャンセルモーダル
- [x] ステータス遷移API呼び出し
- [x] 加工指示書印刷ページ
