...
預かり登録は「商品先行」のウィザード形式。内部的には6つの論理ステップで構成されるが、画面上のステップインジケーターでは4ステップ（撮影 / 商品 / 顧客 / 確認）を短縮ラベルで表示する。
...
- `addMore`ステップは「顧客」と同じインジケーター位置（index 2）に割り当て
- `complete`ステップはインジケーター表示範囲外（index 4）
...
商品情報入力後、顧客選択をスキップした場合に下書きとして保存される。

- **保存タイミング**: 確認ステップで登録実行時に顧客未選択の場合
- **ステータス**: `draft`（顧客未設定）
...
```text
[商品情報入力] → [顧客選択スキップ] → [確認・登録] → [下書き保存（draft）]
                                                          ↓
                                                [ダッシュボードに表示]
                                                          ↓
                                                [クリックで顧客紐付け再開]
...
- 商品種別・商品名は必須入力（バリデーションあり）
- 色・柄と素材は1点目のみ表示（2点目以降は非表示）
- 状態メモと加工指示は統合テキストエリアで入力
- 依頼先業者は任意選択（受付時に決まっている場合に選択）
...
- **続けて登録**: 完了後に新しい受付を開始（状態を初期化して写真撮影から再開）
...
- 生成: `getJSTTimestamp()` 使用（クライアント側で生成）
...
| エンドポイント | メソッド | 説明 | 状態 |
| -------------- | -------- | ---- | ---- |
| /api/receptions | POST | 受付登録（商品一括作成） | 実装済み |
| /api/receptions/draft | POST | 下書き保存（顧客未設定） | 実装済み |
| /api/receptions/[receptionNumber]/customer | PATCH | 下書きへの顧客紐付け | 実装済み |
| /api/customers | GET | 顧客検索 | 実装済み |
| /api/customers | POST | 顧客新規登録 | 実装済み |
...
| コンポーネント | ファイル | 役割 |
| -------------- | -------- | ---- |
| ReceptionWizard | `src/components/reception/ReceptionWizard.tsx` | ウィザード全体の状態管理・ステップ遷移 |
| StepPhoto | `src/components/reception/StepPhoto.tsx` | 写真撮影（カメラ/ファイルアップロード） |
| StepItemDetails | `src/components/reception/StepItemDetails.tsx` | 商品情報入力（業者選択含む） |
| StepCustomer | `src/components/reception/StepCustomer.tsx` | 顧客・取引先選択 |
| StepAddMore | `src/components/reception/StepAddMore.tsx` | 追加登録確認 |
| StepConfirm | `src/components/reception/StepConfirm.tsx` | 登録内容確認・備考入力 |
| StepComplete | `src/components/reception/StepComplete.tsx` | 完了表示・加工指示書出力 |
